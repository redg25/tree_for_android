...
def get_tree_from_tag(tag:str)->json:
    data={'tag':tag}
    response = client.get('http://127.0.0.1:8000/treeapi/trees/', params=data, headers={"X-CSRFToken":csrftoken})
    return response.json()

def update_tree(tag:str,new_values:Dict):
    id = get_tree_from_tag(tag)[0]['id']
    data_patch = new_values
    response = client.patch(f'http://127.0.0.1:8000/treeapi/trees/{str(id)}/', data=data_patch, headers={"X-CSRFToken":csrftoken})

def create_tree(values:Dict):
    response = client.post('http://127.0.0.1:8000/treeapi/trees/', values, headers={"X-CSRFToken":csrftoken})
